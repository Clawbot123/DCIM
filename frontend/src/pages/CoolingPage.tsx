    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl w-full max-w-md shadow-2xl border border-gray-200">
        <div className="flex items-center justify-between px-5 py-3 border-b border-gray-200">
          <h2 className="text-sm font-bold text-gray-800">{editing ? 'Edit Cooling Unit' : 'Add Cooling Unit'}</h2>
        ...
        <div className="p-5 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="block text-xs text-gray-500 mb-1">Cooling Capacity (kW)</label>
            ...
            <div>
              <label className="block text-xs text-gray-500 mb-1">Power Draw (kW)</label>
              <input type="number" className="w-full border border-gray-300 rounded px-3 py-1.5 text-sm text-gray-800 bg-white"
                value={form.power_draw_kw} onChange={e => f('power_draw_kw', Number(e.target.value))} min={0} />
            </div>
          </div>
          {mutation.isError && (
            <div className="text-xs text-red-600 bg-red-50 rounded px-3 py-2">Failed to save cooling unit.</div>
          )}
        </div>
